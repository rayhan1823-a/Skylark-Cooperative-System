    ...
    <div className="p-6 max-w-7xl mx-auto">
      <div className="bg-white rounded-xl shadow-lg p-6">
        ...
        <form onSubmit={handleSubmit} className="grid md:grid-cols-2 gap-4">
          <select name="member" value={formData.member} onChange={handleChange} className="border rounded-lg p-3 bg-white" required>
          ...
          <select name="status" value={formData.status} onChange={handleChange} className="border rounded-lg p-3 bg-white">
                ...
                filteredLoans.map((loan, index) => {
                  ...
                  return (
                    <tr key={loan._id} className="hover:bg-gray-50">
                      <td className="border p-3"><b>{loan.member?.memberId}</b><br />{loan.member?.name}</td>
                      <td className="border p-3 font-semibold text-green-600">৳ {loan.amount}</td>
                      <td className="border p-3">{loan.issueDate ? new Date(loan.issueDate).toLocaleDateString() : "N/A"}</td>
                      <td className="border p-3">{loan.dueDate ? new Date(loan.dueDate).toLocaleDateString() : "N/A"}</td>
